  ...
  const handleCreateOrEditPost = async (e) => {
    ...
    try {
      ...
      if (editingPostId) {
        ...
        // Show success message and trigger refresh in Posts
        alert("✅ Post created successfully! It will appear in the Posts feed shortly.");

        // Trigger a custom event to notify Posts component
        window.dispatchEvent(new CustomEvent('postCreated', {
          detail: { postId: res.data.id, title: res.data.title }
        }));
      }
      setNewPost({
        title: "",
